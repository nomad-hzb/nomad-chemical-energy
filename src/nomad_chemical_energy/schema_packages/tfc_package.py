...
class TFC_Sputtering(MultiTargetSputtering, PlotSection, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=['layer',
                  ...
                  'location'],
            properties=dict(
                order=['name', 'data_file', 'datetime',
                       'substrate', 'sample_owner', 'process_user',
                       'holder', 'sample_lab_label'])))
    ...
    def make_targets_process_table(self):
        ...
        fig = go.Figure(data=[go.Table(header=dict(values=header_values,
                                       ...
                                       cells=dict(values=[value_list, target_names*3, *cells],
    ...
    def normalize(self, archive, logger):
        ...
        self.figures = [PlotlyFigure(label='Table for Target & Process View', figure=fig1.to_plotly_json()), ]
