...
ce_necc_xlsx_parser = CENECCxlsxParserEntryPoint(name='CENECCxlsxParser', description='Parser for CENECC xls files', mainfile_name_re=r'^.*\.xlsx$', mainfile_mime_re='(application|text)/.*')

ce_nome_gamry_parser = CENOMEGamryParserEntryPoint(name='CENOMEGamryParser', description='Parser for CENOME Gamry files', mainfile_name_re=r'^(.*\.DTA)$')

ce_nome_csv_parser = CENOMEcsvParserEntryPoint(name='CENOMEcsvParser', description='Parser for CENOME csv files', mainfile_name_re=r'^(.+\.(pump|oxy)\.(csv|xlsx))$', mainfile_mime_re='(application|text)/.*')

ce_nome_uvvis_parser = CENOMEUVvisParserEntryPoint(name='CENOMEUVvisParser', description='Parser for CENOME uvvis files', mainfile_name_re=r'^(.*\.(csv|ABS))$', mainfile_contents_re=r'^(.*)(\r\n|\r|\n)((WL\/nm,Abs)|("  ABSOR->  Wave:.*))')

ce_nome_xas_parser = CENOMEXASParserEntryPoint(name='CENOMEXASParser', description='Parser for CENOME xas kmc2 files', mainfile_name_re=r'^(.*(\.dat))', mainfile_contents_re='/home/kmc2/data/')

ce_nome_tif_parser = CENOMETIFParserEntryPoint(name='CENOMETIFParser', description='Parser for CENOME tif files', mainfile_name_re=r'^(.*(\.tif|\.tiff))', mainfile_mime_re='image/.*')

ce_nome_massspectrometry_parser = CENOMEMassspectrometryParserEntryPoint(name='MassspectrometryParser', description='Parser for CE-NOME Massspectrometry files', mainfile_name_re=r'^(.*(\.txt))', mainfile_contents_re='^.*Spectra International Data File')

ce_wannsee_mpt_parser = CEWannseeMPTParserEntryPoint(name='CEWannseeMPTParser', description='Parser for Wannsee mpt files', mainfile_name_re=r'^.*\.mpt$')

ce_wannsee_cor_parser = CEWannseeCORParserEntryPoint(name='CEWannseeCORParser', description='Parser for Wannsee cor files', mainfile_name_re=r'^.*\.cor$')

ce_wannsee_xrd_xy_parser = CEWannseeXRDParserEntryPoint(name='CEWannseeXRDParser', description='Parser for Wannsee xy xrd files', mainfile_name_re=r'^(.+\.?.+.xy)$')

dlr_ec_cv_parser = DLRECCVParserEntryPoint(name='DLRECCVParser', description='Parser for DLR CV files', mainfile_contents_re=r'^.*\nPotential\sapplied\s\(V\)\tTime\s\(s\)\tWE\(1\).Current\s\(A\)\tScan\tIndex')

dlr_ec_cp_parser = DLRECCPParserEntryPoint(name='DLRECCPParser', description='Parser for DLR CP files', mainfile_contents_re=r'^(.*)\nTime\s\(s\)\tWE\(1\).Potential\s\(V\)\tCorrected\stime\s\(s\)\tIndex')

dlr_ec_eis_parser = DLRECEISParserEntryPoint(name='DLRECEISParser', description='Parser for DLR EIS files', mainfile_contents_re="""^.*\nIndex\tFrequency\s\(Hz\)\tZ'\s\(Ω\)\s-Z''\s\(Ω\)\tZ\s\(Ω\)\s-Phase\s\(°\)\tTime\s\(s\)""")

hzb_catlab_parser = CatlabParserEntryPoint(name='CatlabParser', description='Parser for Catlab files', mainfile_name_re=r'^.*CatID[0-9].*#.*$', mainfile_mime_re='.*/.*')
